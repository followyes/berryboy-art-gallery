...
assert.ok(!sql.includes('can_remove_prepared_shared_asset_storage_path') && !sql.includes('shared_assets_prepared_delete_select') && !sql.includes('gallery_artworks_prepared_shared_asset_delete_select'));
assert.ok(sql.includes('gallery_artworks_public_select') && sql.includes('gallery_artworks_authenticated_select'));
assert.ok(!admin.includes('const preparedStorageSupabase = createClient') && !assetWorkspace.includes('preparedStorageSupabase'));
assert.ok(!sharedAssetApi.includes('preparedStorageClient') && sharedAssetApi.includes('removeStorageItemsOrThrow(supabase'));
...
console.log('V14.2.5 Gallery/Exhibition safe deletion + V14.4.7.5 authenticated Shared Asset Storage deletion regression passed.');
