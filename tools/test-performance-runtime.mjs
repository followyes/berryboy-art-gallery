...
expect('current package keeps transition guard regression coverage', pkg.version.includes('v14-4-4-shared-asset-gallery-version-gc-authority') && viewer.includes('const STAGE = "V14.1.10.1"') && admin.includes('const STAGE = "V14.1.10.1"'));
...
expect('exhibition switch is guarded', admin.includes('title: options.guardTitle || `Switching to ${target.name}…`') && admin.includes('await sceneLifecycleController.switchTo'));
...
expect('stage identity', source.includes('stage: "C6C8C21"') && source.includes('exhibition-platform-multi-exhibition.v10'));
...
expect('package identity', pkg.version.includes('v14-4-4-shared-asset-gallery-version-gc-authority'));
...
expect("package stage", pkg.version.includes("v14-4-4-shared-asset-gallery-version-gc-authority"));
...
expect('legacy pending snapshot leaves models to the terminal walkthrough batch', !pending.includes('criticalModelQueue') && !pending.includes('modelActive:') && pending.includes('backgroundModelQueue') && source.includes('waitForGalleryWalkthroughFinalSettle('));
...
expect('legacy Preview snapshot delegates visible models to walkthrough terminal batch', !snapshot.includes('snapshot.modelQueue === 0') && !snapshot.includes('snapshot.modelActive === 0') && source.includes('scheduleGalleryWalkthroughHeavyHydration("sculpture-models"'));
...
expect('Props remain structurally optional but assigned Props are policy blocking', source.includes('galleryHasOptionalProps') && source.includes('galleryPolicyPreviewBlockingAssetNames') && source.includes('galleryStartupBlockingAssetNames = galleryPolicyPreviewBlockingAssetNames.slice()'));
...
expect('assigned Props settle in startup gate rather than interaction snapshot blocker', source.includes('propsSettled: !galleryHasOptionalProps') && source.includes('galleryPolicyPreviewBlockingAssetNames.indexOf(assetName) !== -1') && !source.includes('blockers.push("props")'));
