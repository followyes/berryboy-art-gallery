...
expect('current package keeps transition guard regression coverage', pkg.version.includes('v14-1-9-preinteraction-walkthrough-hydration') && viewer.includes('const STAGE = "V14.1.9"') && admin.includes('const STAGE = "V14.1.9"'));
...
expect('legacy pending snapshot leaves models to V14.1.9 walkthrough batch', !pending.includes('criticalModelQueue') && !pending.includes('modelActive:') && pending.includes('backgroundModelQueue') && source.includes('waitForGalleryWalkthroughFinalSettle('));
