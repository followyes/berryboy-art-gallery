...
expect('Core Admin release identity remains V14.1.10 while V14.3.7 UI stage is explicit', admin.includes('const STAGE = "V14.1.10.1"') && admin.includes('ADMIN_PRODUCT_MODEL_STAGE = "V14.3.7"'));
...
expect('V14.3.7 normal Gallery UI exposes product actions rather than technical Version controls', detailRenderer.includes('editing ? "EDITING" : "EDIT"') && detailRenderer.includes('PUBLISH CHANGES') && detailRenderer.includes('PUBLISHED: ON') && detailRenderer.includes('SET START POSITION') && detailRenderer.includes('ADJUST') && !detailRenderer.includes('CREATE NEXT VERSION') && !detailRenderer.includes('EDIT DRAFT') && !detailRenderer.includes('DISCARD DRAFT') && !detailRenderer.includes('VALIDATE DRAFT') && !detailRenderer.includes('PUBLISH VERSION') && !detailRenderer.includes('ROLLBACK') && !detailRenderer.includes('Version history'));
...
expect('V14.3.7 camera-first start position keeps numeric adjustment behind progressive disclosure', detailRenderer.includes('SET START POSITION') && detailRenderer.includes('galleryEntryAdjustPanel') && detailRenderer.includes('class="hidden"') && detailRenderer.includes('SAVE ADJUSTMENT') && testHtml.includes('SET START POSITION'));
...
expect('Gallery Preview has isolated shell and product-facing start-position action', testHtml.includes('data-gallery-test="true"') && testHtml.includes('Gallery preview') && testHtml.includes('SET START POSITION'));
...
console.log('V14.3.7 Unified Admin Product Model Gallery invariants passed.');
