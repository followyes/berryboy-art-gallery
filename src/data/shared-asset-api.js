...
export function createSharedAssetApi({ supabase }) {
  ...
  async function gcVersion(assetVersionId) {
    ...
    // never reactivate the technical version if Storage removal is partial. The same
    // authenticated Admin client performs the Storage removal so Storage SELECT/DELETE
    // RLS sees the actor that created gc_pending_at through the guarded RPC.
  ...
  async function deletePermanent(assetId) {
    ...
    // Whole-Asset Storage cleanup stays on the authenticated Admin client. Storage.remove()
    // requires both SELECT and DELETE RLS for the exact object, so the database contract
    // explicitly grants those capabilities to authenticated only after prepare has marked
    // the Asset deletion-pending.
    await removeStorageItemsOrThrow(supabase, plan.storageItems || plan.storage_items || []);
