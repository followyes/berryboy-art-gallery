...
import { createExhibitionDataAdapter, resolveInitialPublicRuntime, listPublicExhibitionCards } from "../data/exhibition-api.js?v=v14_4_7_2_gallery_version_rebase_preservation";
...
const ENGINE_CACHE_KEY = "v14_4_7_2_gallery_version_rebase_preservation_20260918";
...
function ensureInlineAdminWorkspaceStyles() {
  ...
  style.textContent = `
    ...
    #inlineAdminWorkspace .adminButton.iconButton { width:36px; min-width:36px; padding:0; font-size:16px; line-height:1; }
    ...
    #inlineAdminWorkspace #saveMetadataButton { position:sticky; bottom:0; z-index:5; width:100%; min-height:48px; margin-top:4px; box-shadow:0 -10px 24px rgba(11,13,12,.72),0 12px 24px rgba(49,95,52,.14); }
    #inlineAdminWorkspace #saveMetadataButton[data-save-state="clean"], #inlineAdminWorkspace #saveMetadataButton[data-save-state="saved"] { opacity:.58; box-shadow:none; }
    #inlineAdminWorkspace #saveMetadataButton[data-save-state="dirty"] { background:#315f34; border-color:rgba(154,180,155,.58); opacity:1; }
    #inlineAdminWorkspace #saveMetadataButton[data-save-state="saving"] { opacity:.74; cursor:progress; }
    #inlineAdminWorkspace #saveMetadataButton[data-save-state="failed"] { background:#873c3c; border-color:rgba(209,139,139,.58); opacity:1; }
  ...
  shell.innerHTML = `
    ...
    <div id="inlineAdminBody">
      <aside id="inlineAdminSidebar">
        <section class="workspaceSection"><div class="sectionHead"><div><h2>Exhibitions</h2><p>Create or edit an Exhibition.</p></div><button id="refreshExhibitionsButton" class="adminButton" type="button" title="Refresh">↻</button></div><div class="sectionBody"><form id="createExhibitionForm"><input id="newExhibitionName" class="adminInput" maxlength="120" placeholder="New exhibition name" autocomplete="off"/><select id="newExhibitionGallery" class="adminInput" aria-label="Gallery" required><option value="">Loading Galleries…</option></select><button id="createExhibitionButton" class="adminButton primary" type="submit" disabled>CREATE</button></form><div style="height:10px"></div><div id="exhibitionList"><div class="fieldMeta">Loading exhibition catalog…</div></div></div></section>
        <section class="workspaceSection"><div class="sectionHead"><div><h2>Exhibition</h2><p>Edit content, visibility and cover.</p></div></div><div class="sectionBody"><form id="detailsForm"><label class="fieldLabel">Name<input id="exhibitionName" class="adminInput" maxlength="120" required/></label><label class="fieldLabel">Description<textarea id="exhibitionDescription" class="adminTextarea" maxlength="4000" placeholder="Short exhibition description"></textarea></label><input id="exhibitionSlug" type="hidden"/><label class="fieldLabel">Order<input id="exhibitionSortOrder" class="adminInput" type="number" step="1"/></label><div class="fieldLabel">Poster / cover</div><div class="posterCard"><img id="posterPreview" alt="Exhibition poster preview"/><div class="posterActions"><button id="choosePosterButton" class="adminButton" type="button">UPLOAD / REPLACE</button><button id="removePosterButton" class="adminButton danger" type="button">REMOVE</button><div id="posterStatus" class="fieldMeta">No poster assigned.</div><input id="posterFileInput" type="file" accept="image/jpeg,image/png,image/webp,image/avif"/></div></div><div class="fieldMeta">Gallery: <strong id="exhibitionSpaceId">—</strong></div><button id="saveMetadataButton" class="adminButton primary" type="submit" data-save-state="clean" disabled>ALL CHANGES SAVED</button><div class="publicationControls"><div id="exhibitionPublicationNotice" class="fieldMeta publicationNotice hidden"></div><div class="publicationActions"><button id="toggleExhibitionPublishedButton" class="adminButton" type="button" aria-pressed="false">PUBLISHED: OFF</button><button id="deleteExhibitionButton" class="adminButton danger iconButton" type="button" aria-label="Delete" title="Delete Exhibition">🗑</button></div><div id="exhibitionPublicationStatus" class="fieldMeta">—</div></div></form></div></section>
      </aside>
...
async function openInlineAdminWorkspace(exhibitionId) {
  ...
  try {
    ...
    if (!inlineAdminModulePromise) inlineAdminModulePromise = import(`./admin-workspace-bootstrap.js?v=v14_4_7_2_gallery_version_rebase_preservation`);
